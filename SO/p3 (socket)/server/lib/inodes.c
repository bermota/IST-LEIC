 ...
 *   -1: if an error occurs
 */
int inode_get(int inumber, uid_t *owner, permission *ownerPerm,
              permission *othersPerm, char *fileContents, int len) {
  lock_inode_table();
  if ((inumber < 0) || (inumber > INODE_TABLE_SIZE) ||
      (inode_table[inumber].owner == FREE_INODE)) {
...
int inode_isOpen(int inumber) { return inode_table[inumber].isOpen; }
...
int inode_isFree(int inumber) {
  int isFree;
  lock_inode_table();
  isFree = inode_table[inumber].owner == FREE_INODE;
  unlock_inode_table();
  return isFree;
}